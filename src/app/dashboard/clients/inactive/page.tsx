...
/**
 * Client Component de página para exibição exclusiva de clientes inativos na fábrica.
 * Consulta a base geral de clientes corporativos via Server Action e aplica filtragem
 * em tempo de execução para renderizar apenas empresas com status 'INACTIVE'.
 */
export default function InactiveClientsPage() {
  ...
  async function loadClients() {
    try {
      ...
      // FILTRAÇÃO ESTRITA: Mostra apenas os INATIVOS nesta página
      setClients(data.filter((c: any) => c.status === "INACTIVE"));
    } catch (error) {
    ...
    <div className="flex-1 space-y-4 p-8 pt-6">
      <div>
        <h2 className="text-3xl font-bold tracking-tight">
          Clientes Corporativos Inativos
        </h2>
        <p className="text-sm text-muted-foreground">
          Listagem focada apenas em parceiros comerciais inativos.
        </p>
      </div>
